...
已经在用 [cc-switch](https://github.com/farion1231/cc-switch)？检测到 cc-switch 数据库时，ccc 会直接使用它的配置库，无需迁移：

```bash
$ ccc init
✓ 初始化完成
✓ 检测到 cc-switch，ccc 将直接使用 cc-switch 配置库
```
...
ccc use or  # 等同于: ccc use OpenRouter
```
...
- `~/.claude/settings.json` — 可能仍包含 ccc 写入的 `env` 配置
